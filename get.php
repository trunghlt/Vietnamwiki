...
if(isset($_POST['ok'])){
	...
	$re = mysql_query('select user_id,post_id from editions where checked=0 and post_id!=0');
	while($row = mysql_fetch_assoc($re))
	{
		//$re2 = mysql_query("select post_id, user_id from follow where post_id=$row[post_id] and user_id=$row[user_id]");
	...
	//$re = mysql_query('select id from users where level=1');
	//while($row = mysql_fetch_assoc($re))
	//{
