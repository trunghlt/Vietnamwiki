...
require_once("./libraries/facebook-client/facebook.php");
require_once("./libraries/facebook-client/core.php");

/* Check if the user logged in using Facebook connect */
function chkFbLoggedIn() {
	$fbUserIds = facebook_client()->get_loggedin_user();
	return ($fbUserIds)? true : false;
}
  ...
  $html = sprintf(
    '<script src="%s/js/api_lib/v0.4/FeatureLoader.js.php" type="text/javascript"></script>
     <script type="text/javascript">
       FB.init("%s", "xd_receiver.htm");
     </script>
     <script src="/js/fbconnect.js" type="text/javascript"></script>',
    get_static_root(),
    get_api_key());
